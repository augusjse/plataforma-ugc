...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabaseAdmin.from("videos_ugc").update({ moderation_status: "aprovado" }).eq("id", id).eq("moderation_status", "pendente").select("id,moderation_status").maybeSingle();
